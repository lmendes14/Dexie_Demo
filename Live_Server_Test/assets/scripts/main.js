...
const router = new Router(function () {
  ...
  document.getElementById('create-wrapper').classList.add('shown');
  document.getElementById('delete-wrapper').classList.add('shown');
});
...
async function init() {
  ...
  if (localStorage.getItem("file")){
    //await db.delete();
    console.log("success");
    db = await importDB(dataURItoBlob(localStorage.getItem("file")));
  }else{
  ...
  createButton.addEventListener('click', () => {
    let textElement = document.getElementById('create');
    createRecipe(textElement.value)
    .catch(() => {
      console.log('fail'); 
    });
  ...
  deleteButton.addEventListener('click', () => {
    let textElement = document.getElementById('delete');
    deleteRecipe(textElement.value)
    .catch((error) => {
      console.log('fail');
    });
  });
  ...
  //download(blob, "dexie-export.json");
  db = await importDB(dataURItoBlob(localStorage.getItem("file")));
  */
}
...
async function createRecipe(url) {
  return new Promise((resolve, reject) => {
    ...
    db.recipes.get(url).then((recipe) => {
      if (recipe != undefined){
      ...
      fetch(url)
          .then(response => response.json())
          .then((data) => {
            data['page-name'] = url.split('/').pop().split('.')[0];
            ...
            router.addPage(page, function() {
              ...
              document.getElementById('create-wrapper').classList.remove('shown');
              document.getElementById('delete-wrapper').classList.remove('shown');
              document.querySelector('recipe-expand').data = data;
            });
...
async function deleteRecipe(url) {
  return new Promise((resolve, reject) => {
    ...
    db.recipes.get(url).then((recipe) => {
      ...
      db.recipes.delete(url)
      .then(() => {
        let card = document.getElementById(url);
        document.querySelector('.recipe-cards--wrapper').removeChild(card);
      })
      .then(async () => {
        const blob = await exportDB(db);
...
async function createRecipeCards(){
  ...
  // being used as the key for the recipeData object
    recipeCard.data = recipe.recipe_data;
    recipeCard.id = recipe.recipe_name;
    ...
    const page = recipe.recipe_data['page-name'];
    router.addPage(page, function() {
      document.querySelector('.section--recipe-cards').classList.remove('shown');
      document.querySelector('.section--recipe-expand').classList.add('shown');
      document.getElementById('create-wrapper').classList.remove('shown');
      document.getElementById('delete-wrapper').classList.remove('shown');
      document.querySelector('recipe-expand').data = recipe.recipe_data;
    });
    bindRecipeCard(recipeCard, page);
